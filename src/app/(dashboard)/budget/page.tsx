...
const ensureCategories = (data: BudgetData): BudgetData => {
  const allCategories = [
    "Other", "Bills", "Food", "Entertainment", "Transportation", "EMI",
    "Healthcare", "Education", "Investment", "Shopping", "Fuel", "Groceries",
  ]
  data.monthlyData.forEach((monthData) => {
    allCategories.forEach((category) => {
      if (!monthData.categoryExpenses.hasOwnProperty(category)) {
...
export default async function Page() {
  const [data, budget] = await Promise.all([GetCategoryDataDb(), GetBudgetDb()])
  const data1 = ensureCategories(data)
  ...
  return (
    <div>
      <h1>Budget Page</h1>
      <BudgetSelection budget={Number(budget.budget)} initialData={data1} />
    </div>
  )
}